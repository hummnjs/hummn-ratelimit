...
describe("HummnRatelimit", () => {
  let redis: RedisClient;
  const testPrefix = "test:ratelimit";
  ...
  afterEach(async () => {
    // Clean up test keys
    const keys = await redis.send("KEYS", [`${testPrefix}*`]);
  ...
  describe("Prefix Configuration", () => {
    test("should use custom prefix", async () => {
      ...
      const keys = await redis.send("KEYS", [`${customPrefix}*`]);
    ...
    test("should use default prefix when not specified", async () => {
      ...
      const keys = await redis.send("KEYS", ["@hummn/ratelimit*"]);
      expect(Array.isArray(keys)).toBe(true);
      expect((keys as string[]).length).toBeGreaterThan(0);
